...
class Conv_BN_Act(tf.keras.layers.Layer):
    ...
    def call(self, x, skip_input=None):
        x = self.conv(x)
        x = self.bn(x) if self.is_bn else x
        x = x if self.erase_act else self.act(x)
        if not skip_input==None:
            #print('up concat x {}'.format(x.shape))
            #print('up concat skip_input {}'.format(skip_input.shape))
            x = self.concat([x,skip_input])
            #print('after up concat x {}'.format(x.shape))
        return x
...
class NetG(tf.keras.Model):
    ...
    def call(self, x):
        ...
        latent_o,_ = self.encoder2(gen_img)
...
class Skip_GANomaly(GANRunner):
    def __init__(self,
        ...
        super(Skip_GANomaly, self).__init__(self.G,
                                       self.D,
